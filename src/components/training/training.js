...
export default function Training() {
  ...
  const [selectedRecruitmentPlan, setSelectedRecruitmentPlan] =
    useState(planName);
  ...
  const fetchListInternSelect = async (newPagination = pagination) => {
    ...
    if (user != null) {
      try {
        ...
        axios
          .get(
            `http://localhost:8080/api/interns/searchValue?page=${newPagination.page}&size=${newPagination.size}&keyword=${searchTerm}&trainingState=${selectedTrainingState}&recruitmentPlan=${selectedRecruitmentPlan}`
          )
          .then((res) => {
    ...
    if (user != null) {
      try {
        axios.defaults.headers.common["Authorization"] =
          "Bearer " + user.accessToken;
        axios
          .get("http://localhost:8080/api/interns/recruitment_plan")
          .then((res) => {
  ...
  return (
    <>
      ...
      >
        <Box
          m={2}
          style={{ display: "flex", marginBottom: "8px", marginTop: "14px" }}
        >
          <Icon
            style={{ width: 23, height: 23, color: "rgba(0, 0, 0, 0.60)" }}
            icon="ion:book-sharp"
          />
          <p
            style={{
        ...
        >
          <p className="title text-center mb-0">Quản lý đào tạo</p>
        </div>
        <Dialog open={open} onClose={handleClickPracticeClose}>
          <DialogContent
            sx={{
              p: 0,
              position: "relative",
        ...
        <div className=" mt-3">
          <div className="d-flex justify-content-between">
            <div style={{ marginTop: "-14px" }} className="d-flex pl-15">
              ...
              <FormControl
                className="select-form ml-10 status"
                sx={{ minWidth: "300px" }}
              >
              ...
              >
                <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField {...params} label="Kế hoạch tuyển dụng" />
                  )}
                />
              </FormControl>
            </div>
        ...
        <div
          className="content-recruiment position-relative"
          style={{
            borderRadius: "10px",
            minHeight: "650px",
            width: "99.7%",
            ...
            <table className="table_training" style={{}}>
              <div className="no-scrolling">
                <tr
                  ...
                  className="header-tr grey-text "
                >
                  <th
                    className="training-id position-relative"
                    style={{ padding: "8px" }}
                  >
                    <p className=" position-p w-84">STT</p>
                  </th>
                  <th
                    className="training-name position-relative"
                    style={{ padding: "8px" }}
                  >
                    <p className=" position-p w-124">Tên</p>
                  </th>
                  <th style={{ width: "200px" }} className="position-relative">
                    <p className="mt-short-th position-p w-184">Bắt đầu</p>
                  </th>
                ...
                {listInter.map((item, index) => (
                  <tr className="grey-text count-tr" key={item.id}>
                    <td className="training-id" style={{ padding: "8px" }}>
                      {index + 1 + pagination.page * pagination.size}
                    </td>
                    <td>
                      <span className="training-name">{item.internName}</span>
                    </td>
              ...
              <div className="wrapper">
                ...
                >
                  {listSubjectSelect.map((item) => (
                    ...
                    >
                      <p className="position-p w-124">{item.name}</p>
                    </th>
                  ))}
                </tr>
                {listInter.map((item) => (
              ...
              <div className="no-scrolling">
                ...
                >
                  <th className="position-relative">
                    <p className="position-p w-124">Tổng kết</p>
                  </th>

                  <th className="team-score">Đánh giá trên team</th>
                  <th className="position-relative">
                    <p className="position-p w-124">Kết quả</p>
                  </th>
                  <th className=" text-center position-relative">
                    <p className="position-p w-124">Hành động</p>
                  </th>
                </tr>
                {listInter.map((item) => (
                  <tr className="grey-text count-tr" key={item.id}>
                    ...
                    <td className="team-score">
                      {item.scoreInTeam == "" || item.scoreInTeam == null
                        ? "NA"
                        : item.scoreInTeam}
                    </td>
                    <td>
